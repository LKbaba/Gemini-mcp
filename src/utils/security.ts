...
export async function validatePath(
  ...
  if (hasPathTraversal(inputPath)) {
    throw new SecurityError(
      `路径遍历攻击被阻止: "${inputPath}" 包含不允许的路径遍历模式`,
      'PATH_TRAVERSAL',
      inputPath
    );
  }
...
export async function validateFileSize(
  ...
  try {
    ...
    if (size > maxSize) {
      throw new SecurityError(
        `文件大小超限: "${filePath}" 大小为 ${formatBytes(size)}，超过限制 ${formatBytes(maxSize)}`,
        'SIZE_EXCEEDED',
        filePath
      );
    }
...
export function validateFileCount(
  ...
  if (count > maxFiles) {
    throw new SecurityError(
      `文件数量超限: 发现 ${count} 个文件，超过限制 ${maxFiles} 个`,
      'FILE_LIMIT_EXCEEDED'
    );
  }
}
